...
def lagrange(X, y, ind_u = None, epsilon = 0.1, alpha = 0.05, beta = 0.2):
    m, n = X.shape
    statmodel = ut.define_model(y)

    if ind_u is None:
        ind_u = np.concatenate([np.ones(n // 2), np.zeros(n - n//2)]).astype(bool)
#         ind_u = np.concatenate([np.ones(3), np.zeros(n - 3)]).astype(bool)
...
def likelihood_ratio(X, y, ind_u=None, epsilon = 0.1, alpha = 0.05, beta = 0.2):
...
def wald(X, y, ind_u = None, epsilon = 0.1, alpha = 0.05, beta = 0.2):
    m, n = X.shape
    statmodel = ut.define_model(y)
    if ind_u is None:
        ind_u = np.concatenate([np.ones(n // 2), np.zeros(n - n//2)]).astype(bool)
...
def ALC(X, y, k_for_meaning = 100, l = 0.25, alpha = 0.05, begin = None, end = None, num = None, progress_bar = None, multiprocess = False):
    if progress_bar is None:
        progress_bar = list
    if end is None:
        end = X.shape[0] - 1
...
def KL_method(X, y, k_for_meaning = 100, epsilon = 0.01, begin = None, end = None, num = None, progress_bar = None, multiprocess=False):
    if progress_bar is None:
        progress_bar = list
    if end is None:
        end = X.shape[0] - 1
